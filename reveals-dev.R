...
KPf <- function(vg, u, Zmax, radius, model) {
  ...
  if (model == "Prentice") {
    KP <-
      (exp(-1 * b * radius ^ 0.125) - exp(-1 * b * (Zmax * 1000) ^ 0.125))
  } else {
    #Sugita lake model
...
REVEALS <- function(file_name_list, file_name_avg, file_alvc, u, Zmax) {
	...
	for (kt in 1:maxt) { ### LOOP FOR TIMESLICES
		### u_calc
			...
			if (sumpol > 0) {
				...
				for (i in 1:(length(rownames(polcount)) - 2)) {
					...
					for (j in 1:(length(rownames(polcount)) - 2)) {
						if (i != j) {
							ccc2 <- ccc2 + covqa[i, (length(rownames(polcount)) - 2) * (k - 1) + j]/(KPf(avg[i, 2], u, Zmax, radius, model) * KPf(avg[j, 2], u, Zmax, radius, model))
						}
					}
				}
				...
				for (i in 1:(length(rownames(polcount)) - 2)) {
					for (j in 1:(length(rownames(polcount)) - 2)) {
						ccc3 <- ccc3 + covqa[i, (length(rownames(polcount)) - 2) * (k - 1) + j]/(KPf(avg[i, 2], u, Zmax, radius, model) * KPf(avg[j, 2], u, Zmax, radius, model))
					}
					covqak[i, k] <- ccc3
				}
				for (i in 1:(length(rownames(polcount)) - 2)) {
	...
	if (maxk > 1) {
		...
		return(list(V_sites = REVEALS_V, varV_sites = REVEALS_E))
	}
...
REVEALS <-
  function(file_name_list, file_name_avg, file_alvc, u, Zmax) {
    lst <-
      read.table(file_name_list, sep = ",", colClasses = "character")
    maxk <- length(rownames(lst)) ## NO OF SITES
    ...
    for (k in 1:maxk) {
      ...
      if (length(colnames(lst)) > 1) {
        polcount <-
          read.table(
            as.character(file_name_list), sep = ",", row.names = 1, check.names = F, header = T
          )
      } else {
        polcount <-
          read.table(
            as.character(lst[k,]), sep = ",", row.names = 1, check.names = F, header = T
          )
      }
      timeint <- c(timeint, colnames(polcount))
    }
    ...
    avg <-
      read.table(
        file_name_avg, row.names = 1, header = T, sep = ","
      )
    alvc <-
      read.table(file_alvc, sep = ",", row.names = 1, header = T)
    maxt <- length(timeint) # NO OF TIMESLICES
    Vmean <- matrix(0, nrow = length(rownames(avg)), ncol = maxt)
    rownames(Vmean) <- rownames(avg)
    colnames(Vmean) <- timeint
    ...
    for (kt in 1:maxt) {
      ...
      for (k in 1:maxk) {
        ...
        if (sumpol > 0) {
          ...
          for (i in 1:(length(rownames(polcount)) - 2)) {
            qak[i, k] <- qa[i, k] / KPf(avg[i, 2], u, Zmax, radius, model)
            nak[i, k] <-
              na[i, k] / KPf(avg[i, 2], u, Zmax, radius, model)
          }
          sumqak[k] <- sum(qak[, k])
          sumnak[k] <- sum(nak[, k])
        }
      ...
      V <-
        matrix(0, nrow = length(rownames(polcount)) - 2, ncol = maxk)
      rownames(V) <- rownames(avg)
      if (length(colnames(lst)) > 1) {colnames(V) <- file_name_list} else {colnames(V) <- lst[, 1]}
      ...
      varV <-
        matrix(0, nrow = length(rownames(polcount)) - 2, ncol = maxk)
      rownames(varV) <- rownames(avg)
      if (length(colnames(lst)) > 1) {colnames(varV) <- file_name_list} else {colnames(varV) <- lst[, 1]}
      ...
      for (k in 1:maxk) {
        ...
        if (sumpol > 0) {
          ...
          for (i in 1:(length(rownames(polcount)) - 2)) {
            varqak[i, k] <-
              varqa[i, k] / (KPf(avg[i, 2], u, Zmax, radius, model) ^ 2)
          }
          ccc1 <- ccc2 <- 0
          for (i in 1:(length(rownames(polcount)) - 2)) {
            ccc1 <- ccc1 + varqak[i, k]
            for (j in 1:(length(rownames(polcount)) - 2)) {
              if (i != j) {
                ccc2 <-
                  ccc2 + covqa[i, (length(rownames(polcount)) - 2) * (k - 1) + j] / (KPf(avg[i, 2], u, Zmax, radius, model) * KPf(avg[j, 2], u, Zmax,
                                                                                                                                  radius, model))
              }
            }
          }
          ...
          for (i in 1:(length(rownames(polcount)) - 2)) {
            for (j in 1:(length(rownames(polcount)) - 2)) {
              ccc3 <-
                ccc3 + covqa[i, (length(rownames(polcount)) - 2) * (k - 1) + j] / (KPf(avg[i, 2], u, Zmax, radius, model) * KPf(avg[j, 2], u, Zmax,
                                                                                                                                radius, model))
            }
            covqak[i, k] <- ccc3
          }
        ...
        if (nst > 1) {
          ...
          for (bs in 1:1000) {
            ...
            for (k in 1:nst) {
              polcnt <-
                read.table(
                  as.character(lst[btns[bs, k],]), sep = ",", row.names = 1, check.names = F, header = T
                )
              if (polcnt[2, timeint[kt]] == 1) {
                model <- "Prentice"
              } else {
